...
function countVowels(str) {
  ...
  return str.split('').filter((v) => vowels.indexOf(v) >= 0).length;
}
...
function encodeToRot13(str) {
  let result = '';
  const charIn = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.split(
    ''
  );
  const charOut = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split(
    ''
  );
  for (let i = 0; i < str.length; i += 1) {
    const index = charIn.indexOf(str[i]);
    result = `${result}${index < 0 ? str[i] : charOut[index]}`;
  }
  return result;
}
